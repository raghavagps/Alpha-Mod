...
def parse_sequence(sequence: str, sub_index_a: IndexA, sub_index_b: IndexB) -> Tuple[str, List[Tuple[int, str, str, str]]]:
    ...
    while i < n:
        ...
        if ch == "(":
            ...
            resolved_mods.append((residue_position, official_tlc, smiles, parent_letter))
            residue_position += 1
            i = end_idx + 1
            continue
        ...
        if ch == "|":
            ...
            mod_code = f"NEW{new_ncaa_counter}"
            new_ncaa_counter += 1
        ...
        if ch == "{":
            ...
            if prefix == "ptm":
                ...
                official_tlc = _get_tlc(record, context)
                smiles = _get_smiles(record, context)
                resolved_mods.append((residue_position - 1, official_tlc, smiles, preceding_aa))
                i = end_idx + 1
                continue
    ...
    for item in deferred:
        ...
        official_tlc = _get_tlc(record, context)
        smiles = _get_smiles(record, context)
        resolved_mods.append((position, official_tlc, smiles, amino_acid))
